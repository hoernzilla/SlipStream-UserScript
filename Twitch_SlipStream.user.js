...
// @updateURL    https://github.com/hoernzilla/SlipStream-UserScript/raw/main/Twitch_SlipStream.user.js
// @downloadURL  https://github.com/hoernzilla/SlipStream-UserScript/raw/main/Twitch_SlipStream.user.js
// @author       BjOOgle
// @match        *://*.twitch.tv/*
...
// ==/UserScript==
(function() {
    // Überspringe die Injektion in verschachtelten Frames (nested frames), die keine legitimen Twitch-Embed-Kontexte sind.
    // Die Hauptkanal-Seite von Twitch enthält mehr als 5 versteckte, herkunftsübergreifende (cross-origin) Iframes (für Authentifizierung, Analysen,
    // Werbe-SDKs usw.), und Userscript-Manager / uBO injizieren den Code in alle passenden Frames. Jeder davon
    // wird zu einer konkurrierenden vaft-Instanz, die um die Kontrolle über den Player kämpft. Nur der oberste Frame (top frame)
    // hostet den eigentlichen Player auf twitch.tv/KANAL; verschachtelte Hilfs-Frames sind reines Rauschen.
    // Whitelist für die Injektion in verschachtelte Frames: Die drei dokumentierten Embed-Kontexte von Twitch
    // (https://dev.twitch.tv/docs/embed/video-and-clips/) — dies bewahrt Twitch-Streams, die
    // auf Drittanbieter-Seiten eingebettet sind, wo vaft in einem Iframe läuft, dessen übergeordneter Frame (parent) auf
    // einer anderen Origin (Herkunft) liegt.
    // Verwende window.frameElement, um verschachtelte Frames zu erkennen — liefert null im obersten Frame, das Iframe-Element
    // in einem verschachtelten Frame derselben Origin (same-origin) und wirft einen Fehler bei einer anderen Origin (cross-origin).
    // Zuverlässiger als 'window !== window.top', da Tampermonkey das window-Objekt in einen Proxy einpackt,
    // bei dem der strikte Vergleich selbst im obersten Frame "true" zurückgeben kann.
    let _isNested = false;
