...
// @downloadURL  https://github.com/hoernzilla/SlipStream-UserScript/blob/main/Userscript/TWITCH%20SlipStream%20PRO.user.js
// @match        *://*.twitch.tv/*
...
// ============================================================================
// STEUERZENTRUM: Einstellungen + frei platzierbares Zahnrad-Menü
// Ermöglicht das separate Ein-/Ausschalten von Werbeblocker und VOD-Freischaltung.
// Läuft komplett unabhängig von den beiden Original-Skriptteilen und greift nur
// über eine kleine gemeinsame Einstellungs-Schnittstelle (window.__twitchToolsSettings) ein.
// ============================================================================
(function() {
    'use strict';

    const STORAGE_KEY = 'twitchTools:settings';
    const UI_STATE_KEY = 'twitchTools:uiState';
    const SYNC_CHANNEL = 'twitchTools:sync';
    ...
    function saveSettings(settings) {
        try {
            ...
            console.log('[TwitchTools] Einstellungen konnten nicht gespeichert werden', e);
        }
    }
    ...
    function loadUiState() {
        try {
            ...
            if (!raw) return Object.assign({}, DEFAULT_UI_STATE);
            const parsed = JSON.parse(raw);
            const position = parsed && parsed.position;
            return {
                menuOpen: !!(parsed && parsed.menuOpen),
                position: position && isFinite(position.x) && isFinite(position.y)
                    ? { x: Math.max(0, Math.min(1, Number(position.x))), y: Math.max(0, Math.min(1, Number(position.y))) }
                    : null,
            };
        } catch (e) {
    ...
    function saveUiState(state) {
        try {
            ...
            console.log('[TwitchTools] UI-Zustand konnte nicht gespeichert werden', e);
        }
    }
    ...
    function broadcast(type, payload) {
        if (syncChannel) {
            try {
                syncChannel.postMessage({ source: tabId, type: type, payload: payload });
            } catch (e) {}
        }
    }
    ...
    // Öffentliche, minimale Schnittstelle, die von beiden Skriptteilen synchron
    // (vor deren jeweiligem Start) abgefragt wird. Kein DOM-Zugriff nötig,
    // daher sicher bei document-start verfügbar.
    window.__twitchToolsSettings = {
        get: function() { return Object.assign({}, settings); },
        isAdBlockEnabled: function() { return settings.adBlockEnabled; },
        isVodYoinkEnabled: function() { return settings.vodYoinkEnabled; },
    };
    ...
    function updateSettingsControls() {
        if (!ui) return;
        const adBlockToggle = ui.panel.querySelector('#tt-toggle-adblock');
        const yoinkToggle = ui.panel.querySelector('#tt-toggle-yoink');
        const playVodToggle = ui.panel.querySelector('#tt-toggle-playvod');
        if (adBlockToggle) adBlockToggle.checked = !!settings.adBlockEnabled;
        if (yoinkToggle) yoinkToggle.checked = !!settings.vodYoinkEnabled;
        if (playVodToggle) playVodToggle.checked = !!settings.playVodEnabled;
    }

    function setUiState(change, shouldBroadcast) {
        uiState = Object.assign({}, uiState, change);
        saveUiState(uiState);
        applyUiState();
        if (shouldBroadcast) broadcast('ui', uiState);
    }

    function applyIncomingUiState(nextState) {
        if (!nextState || typeof nextState !== 'object') return;
        const position = nextState.position;
        uiState = {
            menuOpen: !!nextState.menuOpen,
            position: position && isFinite(position.x) && isFinite(position.y)
                ? { x: Math.max(0, Math.min(1, Number(position.x))), y: Math.max(0, Math.min(1, Number(position.y))) }
                : null,
        };
    ...
    // ---- UI: Zahnrad unten rechts + aufklappbares Menü, im Twitch-Look ----
    function buildUI() {
        const css = `
            #tt-gear-btn {
                position: fixed;
            ...
            #tt-panel .tt-row:not(:last-child) {
        ...
        panel.innerHTML =
            ...
            '<div class="tt-row">' +
                ...
                '<label class="tt-switch">' +
                    '<input type="checkbox" id="tt-toggle-adblock" ' + (settings.adBlockEnabled ? 'checked' : '') + '>' +
        ...
        gear.addEventListener('click', function() {
            ...
            setUiState({ menuOpen: !uiState.menuOpen }, true);
        });

        document.addEventListener('click', function(e) {
            if (!panel.contains(e.target) && !gear.contains(e.target)) {
                if (uiState.menuOpen) setUiState({ menuOpen: false }, true);
            }
        });

        const dragState = { timer: null, active: false, pointerId: null, offsetX: 0, offsetY: 0, suppressClick: false };
        ...
        function stopDragging() {
            ...
            gear.classList.remove('tt-dragging');
            try { gear.releasePointerCapture(dragState.pointerId); } catch (e) {}
            dragState.suppressClick = true;
            setTimeout(function() { dragState.suppressClick = false; }, 0);
        }

        function moveGear(event) {
            if (!dragState.active) return;
            const maxLeft = Math.max(VIEWPORT_MARGIN, window.innerWidth - GEAR_SIZE - VIEWPORT_MARGIN);
            const maxTop = Math.max(VIEWPORT_MARGIN, window.innerHeight - GEAR_SIZE - VIEWPORT_MARGIN);
            const left = Math.max(VIEWPORT_MARGIN, Math.min(maxLeft, event.clientX - dragState.offsetX));
            const top = Math.max(VIEWPORT_MARGIN, Math.min(maxTop, event.clientY - dragState.offsetY));
            setUiState({
                position: {
                    x: maxLeft === VIEWPORT_MARGIN ? 0 : (left - VIEWPORT_MARGIN) / (maxLeft - VIEWPORT_MARGIN),
                    y: maxTop === VIEWPORT_MARGIN ? 0 : (top - VIEWPORT_MARGIN) / (maxTop - VIEWPORT_MARGIN),
                },
            }, true);
        }

        gear.addEventListener('pointerdown', function(event) {
            ...
            dragState.timer = setTimeout(function() {
                ...
                gear.classList.add('tt-dragging');
                try { gear.setPointerCapture(event.pointerId); } catch (e) {}
            }, HOLD_TO_DRAG_MS);
        });
        gear.addEventListener('pointermove', moveGear);
        gear.addEventListener('pointerup', stopDragging);
    ...
    function applyUiState() {
        ...
        if (!position) {
            ...
            const maxLeft = Math.max(VIEWPORT_MARGIN, window.innerWidth - GEAR_SIZE - VIEWPORT_MARGIN);
            const maxTop = Math.max(VIEWPORT_MARGIN, window.innerHeight - GEAR_SIZE - VIEWPORT_MARGIN);
            const left = VIEWPORT_MARGIN + position.x * (maxLeft - VIEWPORT_MARGIN);
            const top = VIEWPORT_MARGIN + position.y * (maxTop - VIEWPORT_MARGIN);
            gear.style.left = Math.round(left) + 'px';
            gear.style.top = Math.round(top) + 'px';
            gear.style.right = 'auto';
            gear.style.bottom = 'auto';

            const panelWidth = 240;
            const panelHeight = panel.offsetHeight || 130;
            const panelLeft = Math.max(VIEWPORT_MARGIN, Math.min(window.innerWidth - panelWidth - VIEWPORT_MARGIN, left + GEAR_SIZE - panelWidth));
            const panelTop = Math.max(VIEWPORT_MARGIN, Math.min(window.innerHeight - panelHeight - VIEWPORT_MARGIN, top - panelHeight - 8));
    ...
    function setupSynchronization() {
        try {
            ...
            syncChannel.addEventListener('message', function(event) {
                ...
                if (message.type === 'settings') applyIncomingSettings(message.payload);
                if (message.type === 'ui') applyIncomingUiState(message.payload);
            });
        } catch (e) {
            // Das storage-Event darunter bleibt als Fallback aktiv.
        }

        window.addEventListener('storage', function(event) {
            if (event.key === STORAGE_KEY && event.newValue) {
                try { applyIncomingSettings(JSON.parse(event.newValue)); } catch (e) {}
            }
            if (event.key === UI_STATE_KEY && event.newValue) {
                try { applyIncomingUiState(JSON.parse(event.newValue)); } catch (e) {}
            }
        });
...
// ============================================================================
// TEIL 1: Twitch Ad Blocker (with VODs) — v68.6.5-csai — unverändert
// Läuft weiterhin bei document-start, damit fetch/XHR-Hooks vor Twitchs
...
(function() {
    ...
    'use strict';
    const ourTwitchAdSolutionsVersion = 92;// Bumped 91 → 92. v2.6 — VOD worker-fetch branch FULLY REMOVED. Calling `.text()` on a cloned response (even when then returning the original) disrupts the IVS HLS parser's body consumption → segment 403s and Error #2000 on VODs that play fine without the script. The VOD ad-block result is now entirely from the window-scope `edge.ads.twitch.tv` XHR/fetch block, which is what was doing 100% of the work in field testing anyway. Worker fetch hook reverts to upstream live-only behavior. VOD helper functions (getVodIdFromUsherUrl, isVodMediaPlaylistUrl, etc.) stay in the file but are unused — kept for future iteration if a non-stream-breaking approach is found.
    ...
    function declareOptions(scope) {
        ...
        // ---- VOD support ----
        // VOD pre-roll / mid-roll ads on Twitch are CSAI (client-side ad insertion) — they
        // arrive via a separate GQL ad-schedule request and are stitched into the timeline
        // by the player UI, NOT via the chunked m3u8 like live SSAI ads. The vaft worker-
        // fetch architecture has no leverage to remove them. What we DO provide on the VOD
        // path: (1) early-return all live-only heuristics on /vod/ playlists so they can't
        // misfire (addresses pixeltris issues #251 / #473 / #507 — Firefox VOD hang reports);
        // (2) restore DMCA-muted audio via index-muted-<hash>.m3u8 → index-dvr.m3u8 rewrite
        // and body-level -muted.ts → -unmuted.ts fallback; (3) optional VOD-specific player-
        // type override on the GQL videoPlaybackAccessToken request (cycle through 'site',
        // 'embed', 'thunderdome', etc. as a best-effort CSAI mitigation).
        scope.VodEnabled = true;
        scope.VodUnmute = true;// DMCA-muted-segment unmute rewrite (CloudFront/Akamai .m3u8 only)
        scope.VodForceAccessTokenPlayerType = 'site';// Default 'site' — matches Twitch web's default playerType for VOD token requests. DO NOT inherit live's 'popout': VOD CDN segment signatures are bound to the token's playerType context, and popout-tier tokens get the m3u8 fine but get 403 on segment downloads (observed in field v68.6.2). Override via localStorage twitchAdSolutions_vodPlayerType (e.g. 'embed','thunderdome','frontpage') if 'site' itself ever gets ad-laden.
        scope.VodCdnHostFragments = ['cloudfront', 'akamaized.net', '.hls.ttvnw.net', 'vod-secure.twitch.tv', 'vod-metro.twitch.tv', 'vod-pop-cdn'];
        // Per-VOD state, keyed by numeric vod id. Mirrors StreamInfos shape but minimal.
        scope.VodInfos = Object.create(null);
        scope.CurrentVodID = null;// Last seen VOD id (from /videos/<id> path or /vod/<id>.m3u8 URL)
        scope.VodLoggedCsai = false;// Once-per-VOD log gate for CSAI ad-request notice
    }
        ...
        const newWorker = class Worker extends cleanWorker {
            constructor(twitchBlobUrl, options) {
                ...
                const newBlobStr = `
                    ...
                    ${getVodIdFromUsherUrl.toString()}
                    ${isVodMediaPlaylistUrl.toString()}
                    ${rewriteVodUnmuteUrl.toString()}
                    ${rewriteVodUnmuteBody.toString()}
                    ${processVodM3U8.toString()}
                    ${pickVodPlayerType.toString()}
                    ...
                    VodUnmute = ${VodUnmute};
                    VodForceAccessTokenPlayerType = ${VodForceAccessTokenPlayerType ? "'" + VodForceAccessTokenPlayerType + "'" : null};
                    CurrentVodID = ${CurrentVodID ? "'" + CurrentVodID + "'" : null};
                    ...
                    self.addEventListener('message', function(e) {
                        ...
                        } else if (e.data.key == 'UpdateVodID') {
                            // Window scope detected SPA navigation to /videos/<id>; sync into worker
                            // so worker-side VOD playlist processing knows the current id even if
                            // the Usher URL hasn't been observed yet.
                            CurrentVodID = e.data.value;
                            VodLoggedCsai = false;
                        } else if (e.data.key == 'UpdateVodPlayerType') {
                            VodForceAccessTokenPlayerType = e.data.value;
                        } else if (e.data.key == 'UpdateVodUnmute') {
                            VodUnmute = !!e.data.value;
    ...
    function hookWorkerFetch() {
        ...
        fetch = async function(url, options) {
            if (typeof url === 'string') {
                ...
                url = url.trimEnd();
                // ---- VOD branch REMOVED in v68.6.5 ----
                // Earlier versions (v68.5.0 through v68.6.4) intercepted VOD Usher and VOD-CDN
                // m3u8 URLs to detect VOD context and (optionally) rewrite DMCA-muted segments.
                // Every variant of that interception either broke the IVS HLS parser (calling
                // `.text()` on a response clone before returning the original disrupts how the
                // parser consumes the body stream — confirmed in field with Error #2000 / segment
                // 403 on VODs that played fine without the script) or did nothing useful (the
                // DMCA unmute trick only helps a small subset of VODs and was never the user-
                // facing goal here). VOD ads on Twitch are CSAI — they're killed by the
                // window-scope `edge.ads.twitch.tv` XHR/fetch block, not by m3u8 manipulation.
                // So the worker fetch hook is back to live-only behavior, exactly like upstream.
    ...
    // ---- VOD helpers (live in both window and worker scope via blob serialization) ----
    // Return numeric vod id if url is a Twitch Usher VOD master playlist, else null.
    // Handles both /vod/<id>.m3u8 and /vod/v2/<id>.m3u8 forms.
    function getVodIdFromUsherUrl(url) {
        if (typeof url !== 'string') return null;
        const m = url.match(/^https?:\/\/usher\.ttvnw\.net\/vod\/(?:v2\/)?(\d+)\.m3u8/);
        return m ? m[1] : null;
    }
    // Heuristic: is this a VOD media (chunked) playlist URL on a known VOD CDN?
    // Used to apply the DMCA unmute rewrites and to bypass the live-only processM3U8 path.
    function isVodMediaPlaylistUrl(url) {
        if (typeof url !== 'string' || !url.includes('.m3u8')) return null;
        for (let i = 0; i < VodCdnHostFragments.length; i++) {
            if (url.includes(VodCdnHostFragments[i])) return true;
        }
        return false;
    }
    // Rewrite an index-muted-<HASH>.m3u8 playlist URL to its unmuted index-dvr.m3u8 sibling.
    // Twitch's CDN typically keeps the original (unmuted) playlist alongside the DMCA-muted
    // one for ~24h after broadcast end; on older VODs the unmuted variant may have been
    // purged and the rewritten URL will 404, in which case we revert to the original URL.
    function rewriteVodUnmuteUrl(url) {
        if (typeof url !== 'string') return url;
        // Only the playlist URL is rewritten here; segment-level rewrites happen on the body.
        return url.replace(/index-muted-[0-9A-Za-z]+\.m3u8/, 'index-dvr.m3u8');
    }
    // Body-level rewrite for VOD media playlists: turn -muted.ts segment references into
    // the parallel -unmuted.ts (or bare .ts) segments. We try -unmuted first because it's
    // the canonical filename pattern; if a VOD instead uses the bare-.ts twin, the player
    // will 404 on the -unmuted candidate and we fall back to the bare-.ts variant on retry
    // (controlled by VodInfos[vodId].UnmuteVariant; default 'unmuted').
    function rewriteVodUnmuteBody(textStr, variant) {
        if (typeof textStr !== 'string') return textStr;
        if (variant === 'bare') {
            return textStr.replace(/-muted(\.ts)/g, '$1');
        }
        return textStr.replace(/-muted(\.ts)/g, '-unmuted$1');
    }
    // Lightweight processor for VOD media playlists. Intentionally does NOT run any of the
    // live SSAI signifier checks, ad-segment stripping, CleanPlaylistCount logic, or backup-
    // player-swap state — none of those tags appear on VODs (ads are CSAI; see top-of-file
    // comment). Only does the muted→unmuted body rewrite when enabled, and logs once.
    function processVodM3U8(url, textStr) {
        if (!VodEnabled) return textStr;
        if (!textStr || typeof textStr !== 'string') return textStr;
        const vodId = CurrentVodID || 'unknown';
        let info = VodInfos[vodId];
        if (!info) {
            info = VodInfos[vodId] = {
                VodID: vodId,
                LastSeenAt: Date.now(),
                UnmuteVariant: 'unmuted',// flipped to 'bare' if -unmuted segments 404
                LoggedFirstPlaylist: false,
                LoggedUnmuteApplied: false,
                LoggedDmcaMuted: false,
            };
        }
        info.LastSeenAt = Date.now();
        if (!info.LoggedFirstPlaylist) {
            info.LoggedFirstPlaylist = true;
            const isFinalizedVod = textStr.includes('#EXT-X-PLAYLIST-TYPE:VOD') && textStr.includes('#EXT-X-ENDLIST');
            const isLiveEvent = textStr.includes('#EXT-X-PLAYLIST-TYPE:EVENT') || textStr.includes('#EXT-X-TWITCH-ELAPSED-SECS');
            console.log('[AD DEBUG] VOD media playlist seen — vod_id: ' + vodId + ', kind: ' + (isFinalizedVod ? 'finalized-vod' : isLiveEvent ? 'live-event-dvr' : 'unknown'));
        }
        if (VodUnmute && /-muted\.ts/.test(textStr)) {
            if (!info.LoggedDmcaMuted) {
                info.LoggedDmcaMuted = true;
                console.log('[AD DEBUG] VOD has DMCA-muted segments — applying unmute body rewrite (variant: ' + info.UnmuteVariant + ')');
            }
            textStr = rewriteVodUnmuteBody(textStr, info.UnmuteVariant);
            if (!info.LoggedUnmuteApplied) {
                info.LoggedUnmuteApplied = true;
            }
        }
        return textStr;
    }
    // Pick the playerType to use on a videoPlaybackAccessToken (VOD) GQL request.
    // VodForceAccessTokenPlayerType defaults to 'site' in declareOptions — explicitly
    // breaking inheritance from the live ForceAccessTokenPlayerType ('popout' by default).
    // Reason: popout-tier tokens get the VOD m3u8 fine but the segment URLs returned
    // carry CDN signatures bound to the playerType context, and the VOD CDN rejects
    // popout-context segment requests with 403 (observed in field on live-event-dvr VODs).
    // Only fall back to live or hardcoded 'site' if VodForceAccessTokenPlayerType was
    // explicitly cleared.
    function pickVodPlayerType() {
        if (VodForceAccessTokenPlayerType) return VodForceAccessTokenPlayerType;
        return 'site';
    }
            ...
            if (typeof url === 'string') {
                if (url.includes('gql')) {
                    ...
                    if (ForceAccessTokenPlayerType && typeof init.body === 'string' && init.body.includes('PlaybackAccessToken')) {
                        ...
                        // Per-operation playerType override: VOD requests (isVod:true / vodID set /
                        // operationName 'PlaybackAccessToken' with isVod variable) get the VOD-specific
                        // override; live requests get the live override. Without this split, forcing
                        // 'popout' for live also forces it onto VOD requests where 'site' is the
                        // safer default and where 'thunderdome'/'embed' may be useful CSAI escape
                        // hatches.
                        const isVodOp = (b) => !!(b && b.variables && (b.variables.isVod === true || (b.variables.vodID && b.variables.vodID !== '')));
                        // VOD ops use VodForceAccessTokenPlayerType (default 'site'), NOT
                        // the live ForceAccessTokenPlayerType (default 'popout'). Mixing
                        // these produces 403 on segment fetches — see pickVodPlayerType().
                        const pickType = (b) => isVodOp(b) ? (VodForceAccessTokenPlayerType || 'site') : ForceAccessTokenPlayerType;
                        if (Array.isArray(newBody)) {
                            for (let i = 0; i < newBody.length; i++) {
                                const desired = pickType(newBody[i]);
                                if (newBody[i]?.variables?.playerType && newBody[i]?.variables?.playerType !== desired) {
                                    replacedPlayerType = newBody[i].variables.playerType;
                                    newBody[i].variables.playerType = desired;
                                }
                            }
                        } else {
                            const desired = pickType(newBody);
                            if (newBody?.variables?.playerType && newBody?.variables?.playerType !== desired) {
                                replacedPlayerType = newBody.variables.playerType;
                                newBody.variables.playerType = desired;
                            }
                        }
                        if (replacedPlayerType) {
                            const effective = (Array.isArray(newBody) ? newBody[0]?.variables?.playerType : newBody?.variables?.playerType) || ForceAccessTokenPlayerType;
                            console.log(`[AD DEBUG] Replaced '${replacedPlayerType}' player type with '${effective}' player type`);
                ...
                if (url.includes('edge.ads.twitch.tv')) {
                    ...
                    const onVod = !!isVodPathname(document.location.pathname);
                    const key = onVod ? ('vod-' + csaiType) : csaiType;
                    if (!loggedCsaiTypes.has(key)) {
                        loggedCsaiTypes.add(key);
                        if (onVod) {
                            console.log('[AD DEBUG] VOD CSAI ad request detected — type: ' + csaiType + ' (VOD ads are client-side and cannot be blocked via m3u8; vaft VOD path covers DMCA unmute + safe processing only)');
                        } else {
    ...
    // Return numeric VOD id when pathname matches /videos/<id>, else null. The Twitch web app
    // is a SPA so this can change without a hard navigation — see watchVodNavigation below.
    function isVodPathname(pathname) {
        if (typeof pathname !== 'string') return null;
        const m = pathname.match(/^\/videos\/(\d+)(?:[\/?#]|$)/);
        return m ? m[1] : null;
    }
    // Push the current VOD id (or null) into the worker scope. Polls every 500ms because
    // Twitch's SPA performs URL changes via history.pushState which doesn't fire 'popstate',
    // and patching pushState risks conflicts with FFZ/BTTV/7TV. Polling at 500ms is cheap
    // (a regex + string compare) and avoids the patch conflict.
    let lastSentVodId = undefined;// undefined = never sent; null = explicitly cleared
    function syncVodIdToWorker() {
        const id = isVodPathname(document.location.pathname);
        if (id !== lastSentVodId) {
            lastSentVodId = id;
            postTwitchWorkerMessage('UpdateVodID', id);
            if (id) {
                console.log('[AD DEBUG] VOD context entered — vod_id: ' + id + ', VodForceAccessTokenPlayerType: ' + (VodForceAccessTokenPlayerType || '(inherit live)') + ', VodUnmute: ' + VodUnmute);
            }
        }
    }
    function watchVodNavigation() {
        try { syncVodIdToWorker(); } catch {}
        setInterval(syncVodIdToWorker, 500);
    }
    ...
    try {
        ...
        // ---- VOD knobs ----
        const lsVodEnabled = localStorage.getItem('twitchAdSolutions_vodEnabled');
        if (lsVodEnabled === 'false') {
            VodEnabled = false;
            console.log('[AD DEBUG] VOD support disabled via localStorage — all /vod/ + VOD-CDN m3u8 traffic passes through unmodified');
        }
        const lsVodUnmute = localStorage.getItem('twitchAdSolutions_vodUnmute');
        if (lsVodUnmute === 'false') {
            VodUnmute = false;
            console.log('[AD DEBUG] VOD DMCA unmute disabled via localStorage — muted segments stay muted');
        }
        const lsVodPlayerType = localStorage.getItem('twitchAdSolutions_vodPlayerType');
        if (typeof lsVodPlayerType === 'string' && lsVodPlayerType !== '') {
            VodForceAccessTokenPlayerType = lsVodPlayerType;
            console.log('[AD DEBUG] VOD playerType override set via localStorage: ' + lsVodPlayerType);
        }
    } catch {}
    console.log('[AD DEBUG] Config: ReloadPlayerAfterAd = ' + ReloadPlayerAfterAd + ', ForceAccessTokenPlayerType = ' + ForceAccessTokenPlayerType + ', PinBackupPlayerType = ' + PinBackupPlayerType + ', VodEnabled = ' + VodEnabled + ', VodUnmute = ' + VodUnmute + ', VodForceAccessTokenPlayerType = ' + (VodForceAccessTokenPlayerType || '(inherit live)'));
    hookWindowWorker();
    hookFetch();
    watchVodNavigation();
    ...
    // ---- VOD CSAI bypass (v2) ----
    // Based on observed network trace during a VOD pre-roll: Twitch's CSAI flow on VODs is
    // an HTML overlay rendered on top of a paused <video> element, NOT a MediaSource feed.
    // The VOD content is fully buffered the entire ad break. So three intervention points,
    // in order of confidence:
    //   1) Block edge.ads.twitch.tv/2018-01-01/3p/ads XHR + fetch — the ad-creative fetch
    //   2) Neuter VOD ad-schedule GQL responses (AdRequestHandling, VODMidrollManager,
    //      VideoAdBanner) so the player's ad orchestrator never schedules a break
    //   3) Backstop: when the "Ad N of M" overlay mounts, hide it + force video.play()
    (function installVodCsaiBypass() {
        let vodCsaiEnabled = true;
        let blockAdRequests = true;
        // GQL neuter DEFAULT OFF as of v68.6.2 — recorder dump showed the XHR block alone
        // defeats the ad on most channels (it's the gating call), while the GQL neuter on
        // ops like VideoAdRequestDecline produces Apollo error #12 ("Missing field
        // 'adContext' in {}") because the player reads typed fields on the response that
        // {data:{}} doesn't provide. Opt back in with twitchAdSolutions_vodNeuterAdGql=true
        // only when capturing real response shapes (then we can tailor the neuter shape).
        let neuterAdGql = false;
        let hideOverlayAndResume = true;
        try {
            const ls = (k) => localStorage.getItem(k);
            if (ls('twitchAdSolutions_vodCsaiBypass') === 'false') vodCsaiEnabled = false;
            if (ls('twitchAdSolutions_vodBlockAdRequest') === 'false') blockAdRequests = false;
            if (ls('twitchAdSolutions_vodNeuterAdGql') === 'true') neuterAdGql = true;// flipped — opt-in
            if (ls('twitchAdSolutions_vodHideOverlay') === 'false') hideOverlayAndResume = false;
        } catch {}
        if (!vodCsaiEnabled) {
            console.log('[AD DEBUG] VOD CSAI bypass disabled via localStorage');
            return;
        }
        const isOnVod = () => !!isVodPathname(document.location.pathname);
        // ----- Intervention 1: block ad-creative endpoints -----
        // Hosts/paths to block. Returning a 200 with an empty/synthesized body is safer
        // than 404 — the player's error handling sometimes retries on 4xx/5xx, which we
        // don't want. Empty ad list = "no ad available" path.
        // Endpoints that GATE the ad break — synthesize an empty response so the player's
        // ad-decision call resolves with "no ads available" instead of erroring (which
        // would risk a player stall, per research workflow findings).
        const AD_BLOCK_PATTERNS = [
            { host: 'edge.ads.twitch.tv', path: '/' },// long-stable, observed at t=1688ms in the trace
            { host: 'imasdk.googleapis.com', path: '/' },
            { host: 'securepubads.g.doubleclick.net', path: '/' },
            { host: 'pubads.g.doubleclick.net', path: '/' },
        ];
        // Ad-telemetry endpoints — observability, not delivery. Silencing them avoids
        // server-side detection escalation that can cascade into repeated ads (per
        // pixeltris #471 and Twitch's 2023 device-id rate-limiting incident).
        const AD_TELEMETRY_PATTERNS = [
            { host: 'countess.twitch.tv', path: '/' },
            // spade.twitch.tv only when the path mentions ad-related events — it carries
            // legitimate non-ad telemetry that the player wants to send.
            { host: 'spade.twitch.tv', pathRegex: /ad|midroll|preroll|video_ad/i },
        ];
        const shouldBlockUrl = (url) => {
            try {
                const u = new URL(url, location.href);
                return AD_BLOCK_PATTERNS.some((p) => u.host === p.host && u.pathname.startsWith(p.path));
            } catch { return false; }
        };
        const shouldSilenceTelemetry = (url) => {
            try {
                const u = new URL(url, location.href);
                return AD_TELEMETRY_PATTERNS.some((p) => u.host === p.host && (p.path ? u.pathname.startsWith(p.path) : p.pathRegex.test(u.pathname + u.search)));
            } catch { return false; }
        };
        const EMPTY_AD_RESPONSE_JSON = '[]';// Surveyed community work converges on `[]` as the safe "no ads" shape
        const synthesizeBlockedAdResponse = () => {
            return new Response(EMPTY_AD_RESPONSE_JSON, {
                status: 200,
                statusText: 'OK',
                headers: { 'Content-Type': 'application/json' },
            });
        };
        const synthesizeSilencedTelemetryResponse = () => {
            return new Response('', { status: 204, statusText: 'No Content' });
        };
        if (blockAdRequests) {
            // Wrap window.fetch ONCE more on top of the existing hookFetch chain so we run
            // before the real network call. window.realFetch was stashed by hookFetch.
            try {
                const baseFetch = window.fetch;
                window.fetch = maskAsNative(function(url, init) {
                    const u = typeof url === 'string' ? url : (url && url.url) || '';
                    if (shouldBlockUrl(u)) {
                        if (!installVodCsaiBypass._loggedFetchBlock) {
                            installVodCsaiBypass._loggedFetchBlock = true;
                            console.log('[AD DEBUG] Blocked ad-creative fetch: ' + new URL(u, location.href).host);
                        }
                        try {
                            const url2 = new URL(u, location.href);
                            if (window.vaftRecorder) window.vaftRecorder.blockedRequests.push({ t: Date.now() - new Date(window.vaftRecorder.startedAt).getTime(), kind: 'fetch', host: url2.host, path: url2.pathname.slice(0, 200) });
                        } catch {}
                        return Promise.resolve(synthesizeBlockedAdResponse());
                    }
                    if (shouldSilenceTelemetry(u)) {
                        if (!installVodCsaiBypass._loggedTelemetrySilence) {
                            installVodCsaiBypass._loggedTelemetrySilence = true;
                            console.log('[AD DEBUG] Silenced ad-telemetry fetch: ' + new URL(u, location.href).host + new URL(u, location.href).pathname);
                        }
                        try {
                            const url2 = new URL(u, location.href);
                            if (window.vaftRecorder) window.vaftRecorder.blockedRequests.push({ t: Date.now() - new Date(window.vaftRecorder.startedAt).getTime(), kind: 'fetch-telemetry', host: url2.host, path: url2.pathname.slice(0, 200) });
                        } catch {}
                        return Promise.resolve(synthesizeSilencedTelemetryResponse());
                    }
                    return baseFetch.apply(this, arguments);
                }, 'fetch');
            } catch (err) { console.log('[AD DEBUG] VOD CSAI fetch hook failed: ' + err.message); }
            // Wrap XHR — Twitch hits edge.ads.twitch.tv via XHR specifically (confirmed in
            // the trace: GET edge.ads.twitch.tv/2018-01-01/3p/ads). Override .open to
            // rewrite the URL to a never-resolving sentinel so the XHR errors out cleanly,
            // and short-circuit .send so no actual network request is made.
            try {
                ...
                XMLHttpRequest.prototype.open = maskAsNative(function(method, url) {
                    if (typeof url === 'string' && shouldBlockUrl(url)) {
                        this.__vaftBlocked = true;
                        if (!installVodCsaiBypass._loggedXhrBlock) {
                            installVodCsaiBypass._loggedXhrBlock = true;
                            try { console.log('[AD DEBUG] Blocked ad-creative XHR: ' + new URL(url, location.href).host); } catch {}
                        }
                        try {
                            const url2 = new URL(url, location.href);
                            if (window.vaftRecorder) window.vaftRecorder.blockedRequests.push({ t: Date.now() - new Date(window.vaftRecorder.startedAt).getTime(), kind: 'xhr', host: url2.host, path: url2.pathname.slice(0, 200) });
                        } catch {}
                        // Call original .open with a data: URL so the XHR completes with our payload.
                        const dataUrl = 'data:application/json;base64,' + btoa(EMPTY_AD_RESPONSE_JSON);
                        return origOpen.call(this, method, dataUrl, true);
                    }
                    return origOpen.apply(this, arguments);
                }, 'open');
                XMLHttpRequest.prototype.setRequestHeader = maskAsNative(function() {
                    if (this.__vaftBlocked) return;// data: URLs reject custom headers
                    return origSetRequestHeader.apply(this, arguments);
                }, 'setRequestHeader');
                XMLHttpRequest.prototype.send = maskAsNative(function() {
                    return origSend.apply(this, arguments);
                }, 'send');
            } catch (err) { console.log('[AD DEBUG] VOD CSAI XHR hook failed: ' + err.message); }
        }
        // ----- Intervention 2: neuter VOD ad-schedule GQL responses -----
        // The fetch hook above already wraps window.fetch. Add another wrap that, after the
        // real response comes back, inspects the GQL request body to detect ad ops and
        // replaces the response body with a synthetic "no ads" payload.
        //
        // Operations to neuter:
        //   - AdRequestHandling           → "no ad scheduled"
        //   - VODMidrollManager           → "no midrolls"
        //   - VideoAdBanner               → empty banner
        //   - AdRequestHandling_GetAdSession → empty session
        // Mutations to silently 200-OK (don't tell Twitch about ad events):
        //   - ClientSideAdEventHandling_RecordAdEvent
        //   - VideoAdRequestDecline (this is actually a CLIENT mutation telling Twitch the
        //     ad was declined — let it through; it might help mark the slot as filled)
        // GROUND-TRUTH SHAPES from captured response bodies (gqlAdResponses in recorder
        // v68.6.2 dump). The recorder showed exactly the response Twitch sends, so we can
        // synthesize a precise "decline this ad" payload instead of {data:{}}. Key fields:
        //
        //   VideoAdRequestDecline → data.adContext.declineState.shouldDecline = true
        //   This is the gating decision — when true, the player drops the ad break.
        //
        //   ClientSideAdEventHandling_RecordAdEvent → data.recordAdEvent.error = null
        //   It's a fire-and-forget tracking mutation. Returning a success shape silences it.
        const REWRITE_VOD_CSAI_REQUEST_DECLINE = (op) => ({
            data: {
                adContext: {
                    id: 'vaft-synth-' + Math.random().toString(36).slice(2, 10).toUpperCase(),
                    radToken: '',
                    declineState: {
                        reason: 'AD_BLOCKER_DETECTED',
                        shouldDecline: true,
            ...
            extensions: { operationName: 'VideoAdRequestDecline', vaftNeutered: true },
        });
        const REWRITE_CLIENT_SIDE_AD_EVENT = (op) => ({
            data: { recordAdEvent: { error: null, __typename: 'RecordAdEventPayload' } },
            extensions: { operationName: 'ClientSideAdEventHandling_RecordAdEvent', vaftNeutered: true },
        });
        // Per-op neuter table. ONLY ops with known-correct shapes go here. Don't add new
        // ops without first capturing the real response via the recorder and confirming
        // the schema — adding {data:{}} for an op the player reads typed fields on
        // produces Apollo error #12 ("Missing field 'X' in {}") and unmounts the player.
        const NEUTER_TABLE = {
            'VideoAdRequestDecline': REWRITE_VOD_CSAI_REQUEST_DECLINE,
            'ClientSideAdEventHandling_RecordAdEvent': REWRITE_CLIENT_SIDE_AD_EVENT,
        };
        const NEUTER_OPS = new Set(Object.keys(NEUTER_TABLE));
        const SUPPRESS_MUTATION_OPS = new Set();// kept for backward compat — empty now
        const synthesizeNeuteredGqlOp = (op) => {
            const rewriter = NEUTER_TABLE[op?.operationName];
            return rewriter ? rewriter(op) : null;
        };
        if (neuterAdGql) {
            try {
                ...
                window.fetch = maskAsNative(function(url, init) {
                    const u = typeof url === 'string' ? url : (url && url.url) || '';
                    // Only inspect VOD-context GQL POSTs to gql.twitch.tv
                    if (!isOnVod()) return baseFetch2.apply(this, arguments);
                    let host = '';
                    try { host = new URL(u, location.href).host; } catch {}
                    if (host !== 'gql.twitch.tv') return baseFetch2.apply(this, arguments);
                    if (!init || typeof init.body !== 'string') return baseFetch2.apply(this, arguments);
                    let parsedBody;
                    try { parsedBody = JSON.parse(init.body); } catch { return baseFetch2.apply(this, arguments); }
                    const ops = Array.isArray(parsedBody) ? parsedBody : [parsedBody];
                    const hasNeuterOp = ops.some((op) => op && NEUTER_OPS.has(op.operationName));
                    const hasSuppressOp = ops.some((op) => op && SUPPRESS_MUTATION_OPS.has(op.operationName));
                    if (!hasNeuterOp && !hasSuppressOp) return baseFetch2.apply(this, arguments);
                    return baseFetch2.apply(this, arguments).then(async (response) => {
                        try {
                            // Synthesize response body matching the original batch shape
                            let respBody;
                            if (Array.isArray(parsedBody)) {
                                respBody = parsedBody.map((op) => {
                                    if (NEUTER_OPS.has(op?.operationName)) {
                                        return JSON.parse(synthesizeNeuteredGqlResponse(op.operationName));
                                    }
                                    if (SUPPRESS_MUTATION_OPS.has(op?.operationName)) {
                                        return { data: { recordAdEvent: { __typename: 'RecordAdEventPayload', success: true } } };
                                    }
                                    return null;// caller has to re-fetch
                                });
                                // Mixed batch: only safe if EVERY op is a neuter/suppress; if any are
                                // legitimate ops the player needs real data for, fall back to letting
                                // the real response pass through.
                                if (respBody.includes(null)) return response;
                            } else {
                                if (NEUTER_OPS.has(parsedBody.operationName)) {
                                    respBody = JSON.parse(synthesizeNeuteredGqlResponse(parsedBody.operationName));
                                } else if (SUPPRESS_MUTATION_OPS.has(parsedBody.operationName)) {
                                    respBody = { data: { recordAdEvent: { __typename: 'RecordAdEventPayload', success: true } } };
                                } else {
                                    return response;
                                }
                            }
                            const opNames = ops.map((o) => o?.operationName).filter(Boolean).join(',');
                            if (!installVodCsaiBypass._loggedGqlNeuterOps) installVodCsaiBypass._loggedGqlNeuterOps = new Set();
                            if (!installVodCsaiBypass._loggedGqlNeuterOps.has(opNames)) {
                                installVodCsaiBypass._loggedGqlNeuterOps.add(opNames);
                                console.log('[AD DEBUG] Neutered VOD ad GQL ops: ' + opNames);
                            }
            ...
            } catch (err) { console.log('[AD DEBUG] VOD CSAI GQL neuter failed: ' + err.message); }
        }
        // ----- Intervention 3: cosmetic overlay hide (NO force-resume) -----
        // Research confirms (pixeltris #43/#175): calling video.play() during the ad pause
        // is re-overridden by player-core on its next tick, producing pause/unpause spurts
        // that make the user experience WORSE. The pause is the SYMPTOM; defeating it
        // requires preventing the upstream ad-state machine from entering the break, which
        // is what interventions 1 and 2 do. Intervention 3 is detection + cosmetic-hide
        // only — covers the rare case where (1)+(2) leak a single break so the user at
        // least doesn't see the "Ad 1 of 2" overlay.
        //
        // Selectors target stable data-a-target / data-test-selector / .ad-banner /
        // .stream-display-ad attributes (Twitch hash-rotates CSS class names but these
        // identifiers have been stable for years per Harest's twitch-hide-ads userscript).
        if (hideOverlayAndResume) {
            try {
                // Also flip the existing TwitchAdSolutions knob so any in-script overlay
                // suppression (e.g. .tas-adblock-overlay) is also active.
                localStorage.setItem('twitchAdSolutions_hideAdOverlay', 'true');
                const style = document.createElement('style');
                style.id = 'vaft-vod-csai-hide';
                style.textContent = `
                    /* Hide Twitch VOD ad-break overlay variants. Stable data-* selectors. */
                    [data-a-target="ax-overlay"],
                    [data-a-target="video-ad-label"],
                    [data-a-target="advertiser-ad-label"],
                    ...
                    .player-ad-notice,
                    .ad-interrupt-screen,
                    .tas-adblock-overlay { display: none !important; visibility: hidden !important; opacity: 0 !important; pointer-events: none !important; }
                `;
                (document.head || document.documentElement).appendChild(style);
            } catch {}
            // MutationObserver scoped to the player container (or document.body if it
            // hasn't mounted yet) — scoping keeps CPU cost low. Observer is hide-only;
            // no force-resume. If we see the overlay mount, it means (1)+(2) failed to
            // fully suppress this break — log it for diagnostic.
            ...
            const hideAdNodes = (root) => {
                if (!root || !root.querySelectorAll) return;
                const nodes = root.querySelectorAll(AD_OVERLAY_SEL);
                nodes.forEach((n) => {
                    n.style.setProperty('display', 'none', 'important');
                    n.style.setProperty('visibility', 'hidden', 'important');
                    n.setAttribute('data-vaft-hidden', '1');
                });
                if (root.matches && root.matches(AD_OVERLAY_SEL)) {
                    root.style.setProperty('display', 'none', 'important');
                    root.setAttribute('data-vaft-hidden', '1');
                }
            };
            const isAdOverlayNode = (node) => {
                if (!node || node.nodeType !== 1) return false;
                // Match stable selectors first (cheap)
                if (node.matches && node.matches(AD_OVERLAY_SEL)) return true;
                if (node.querySelector && node.querySelector(AD_OVERLAY_SEL)) return true;
                // Fall back to text match (covers selector rotations)
                const text = (node.textContent || '');
                if (text.length > 500) return false;
                return /Ad\s*\d+\s*of\s*\d+|stick around to support|after this ad break/i.test(text);
            };
            const observer = new MutationObserver((muts) => {
                ...
                for (const m of muts) {
                    for (const n of m.addedNodes) {
                        ...
                        if (isAdOverlayNode(n)) {
                            hideAdNodes(n);
                            if (!installVodCsaiBypass._loggedOverlayLeak) {
                                installVodCsaiBypass._loggedOverlayLeak = true;
                                console.log('[AD DEBUG] VOD ad-overlay node detected & hidden — interventions 1+2 may have leaked this break. Check vaftDump() for the GQL ops that fired.');
                            }
                        }
                    }
                }
            ...
            const attachObserver = () => {
                const target = document.querySelector('.video-player__container') ||
                               document.querySelector('.video-player') ||
                               document.body;
                if (!target) { setTimeout(attachObserver, 500); return; }
                hideAdNodes(target);
                try { observer.observe(target, { childList: true, subtree: true }); } catch {}
            };
            if (document.readyState === 'loading') {
        ...
        console.log('[AD DEBUG] VOD CSAI bypass installed — blockAdRequests=' + blockAdRequests + ', neuterAdGql=' + neuterAdGql + ', hideOverlay=' + hideOverlayAndResume);
    })();
    // ---- Diagnostic recorder ----
    // Capture everything an iterating maintainer would need to see (without leaking
    // auth tokens). Call window.vaftDump() in the console to get a JSON report and
    // an auto-download. Enable with `localStorage.twitchAdSolutions_recorder = 'true'`
    // then reload; default OFF so normal users don't pay the overhead.
    (function installDiagnosticRecorder() {
        try {
            if (localStorage.getItem('twitchAdSolutions_recorder') !== 'true') return;
        } catch { return; }
        const T0 = Date.now();
        const t = () => Date.now() - T0;
        const rec = {
            schema: 'vaft-recorder/v1',
            scriptVersion: window.twitchAdSolutionsVersion,
            startedAt: new Date().toISOString(),
            userAgent: navigator.userAgent,
            url: location.href,
            pathname: location.pathname,
            vodIdAtStart: isVodPathname(location.pathname),
            logs: [],            // [AD DEBUG] console lines + everything else interesting
            gqlOps: [],          // {t, operationName, hasVariables, varKeys, isVodOp}
            gqlAdResponses: [],  // {t, operationName, sanitizedBodyKeys, sample}
            fetches: [],         // {t, method, urlHost, urlPath, isGql, isAdsEdge, isImasdk, isUsher, isVodCdn}
            xhrs: [],            // {t, method, urlHost, urlPath}
            mediaSource: [],     // {t, kind, mime?, size?, time?}
            overlays: [],        // {t, kind, text}
            video: [],           // periodic snapshots of <video> state during ads
            blockedRequests: [], // {t, kind, host, path} — what the CSAI bypass blocked
            errors: [],          // captured runtime errors
        };
        const MAX_PER_BUCKET = 2000;// belt-and-suspenders bound on memory
        const push = (bucket, item) => { if (bucket.length < MAX_PER_BUCKET) bucket.push(item); };
        const safeUrl = (u) => {
            try {
                const url = new URL(u, location.href);
                return { host: url.host, path: url.pathname };
            } catch { return { host: '', path: String(u).slice(0, 200) }; }
        };
        const sanitizeGqlBody = (body) => {
            try {
                const parsed = typeof body === 'string' ? JSON.parse(body) : body;
                const ops = Array.isArray(parsed) ? parsed : [parsed];
                return ops.map(op => ({
                    operationName: op?.operationName || '(anonymous)',
                    varKeys: op?.variables ? Object.keys(op.variables) : [],
                    isVodOp: !!(op?.variables && (op.variables.isVod === true || (op.variables.vodID && op.variables.vodID !== ''))),
                    vodID: op?.variables?.vodID || null,
                    hasAdHint: /ad|Ad/.test(op?.operationName || ''),
                }));
            } catch { return null; }
        };
        // Console capture (tee, don't replace — preserve existing console output)
        const origConsoleLog = console.log;
        console.log = function() {
            try {
                const msg = Array.from(arguments).map(a => typeof a === 'string' ? a : JSON.stringify(a, (k, v) => typeof v === 'function' ? '[fn]' : v)).join(' ');
                if (msg.includes('[AD DEBUG]') || msg.includes('CSAI') || msg.includes('VOD')) {
                    push(rec.logs, { t: t(), msg: msg.slice(0, 2000) });
                }
            } catch {}
        ...
        console.error = function() {
            try {
                const msg = Array.from(arguments).map(a => typeof a === 'string' ? a : (a && a.message) || JSON.stringify(a)).join(' ');
                push(rec.errors, { t: t(), msg: msg.slice(0, 2000) });
            } catch {}
            return origConsoleError.apply(this, arguments);
        };
        window.addEventListener('error', (e) => push(rec.errors, { t: t(), msg: (e && e.message) || 'window error', src: e?.filename }));
        window.addEventListener('unhandledrejection', (e) => push(rec.errors, { t: t(), msg: 'unhandledrejection: ' + ((e && e.reason && e.reason.message) || String(e?.reason)) }));
        // Fetch wrap (run AFTER hookFetch installed). We're already wrapping window.fetch,
        // so wrap it once more on top.
        const AD_OP_NAMES_FOR_RESPONSE_CAPTURE = new Set(['AdRequestHandling', 'VODMidrollManager', 'VideoAdBanner', 'VideoAdRequestDecline', 'Ads_Components_AdManager_User', 'ClientSideAdEventHandling_RecordAdEvent', 'AdRequestHandling_GetAdSession', 'PlaybackAccessToken', 'PlaybackAccessToken_Template']);
        const sanitizeResponseSample = (body) => {
            try {
                const obj = typeof body === 'string' ? JSON.parse(body) : body;
                const json = JSON.stringify(obj);
                // Cap each captured sample at 4 KB. Strip auth-looking values defensively.
                return json.replace(/"(access_?token|authorization|cookie|signature|sessionId|session_id)"\s*:\s*"[^"]*"/gi, '"$1":"[REDACTED]"').slice(0, 4096);
            } catch { return null; }
        };
        try {
            const realFetch = window.fetch;
            window.fetch = function(input, init) {
                ...
                try {
                    ...
                    const u = safeUrl(url);
                    const isGql = u.host === 'gql.twitch.tv';
                    const isAdsEdge = u.host === 'edge.ads.twitch.tv';
                    const isImasdk = u.host.includes('imasdk.googleapis');
                    const isUsher = u.host === 'usher.ttvnw.net';
                    const isVodCdn = !!isVodMediaPlaylistUrl(url);
                    const method = (init && init.method) || (typeof input === 'object' && input.method) || 'GET';
                    push(rec.fetches, { t: t(), method, host: u.host, path: u.path.slice(0, 200), isGql, isAdsEdge, isImasdk, isUsher, isVodCdn });
                    if (isGql && init && typeof init.body === 'string') {
                        const sanitized = sanitizeGqlBody(init.body);
                        if (sanitized) {
                            sanitized.forEach(s => push(rec.gqlOps, { t: t(), ...s }));
                            captureOps = sanitized.filter(s => AD_OP_NAMES_FOR_RESPONSE_CAPTURE.has(s.operationName)).map(s => s.operationName);
                        }
                    }
                } catch {}
                ...
                if (captureOps.length > 0) {
                    return p.then(async (response) => {
                        try {
                            ...
                            const sample = sanitizeResponseSample(text);
                            captureOps.forEach((op) => push(rec.gqlAdResponses, { t: t(), operationName: op, sample }));
                        } catch {}
                        return response;
                    }, (err) => { throw err; });
                }
        ...
        } catch (err) { push(rec.errors, { t: t(), msg: 'fetch wrap failed: ' + err.message }); }
        // XHR wrap
        try {
            const origOpen = XMLHttpRequest.prototype.open;
            XMLHttpRequest.prototype.open = function(method, url) {
                try {
                    const u = safeUrl(url);
                    push(rec.xhrs, { t: t(), method, host: u.host, path: u.path.slice(0, 200) });
                } catch {}
                return origOpen.apply(this, arguments);
            };
        } catch (err) { push(rec.errors, { t: t(), msg: 'xhr wrap failed: ' + err.message }); }
        // MediaSource wrap — catches whether ads play through the regular SourceBuffer or
        // whether the player tears down + rebuilds the MediaSource (CSAI ad-switch pattern).
        try {
            const origAdd = MediaSource.prototype.addSourceBuffer;
            MediaSource.prototype.addSourceBuffer = function(mime) {
                push(rec.mediaSource, { t: t(), kind: 'addSourceBuffer', mime: String(mime).slice(0, 100) });
                const sb = origAdd.apply(this, arguments);
                try {
                    const origAppend = sb.appendBuffer;
                    sb.appendBuffer = function(data) {
                        try { push(rec.mediaSource, { t: t(), kind: 'appendBuffer', size: (data && (data.byteLength || data.length)) || 0 }); } catch {}
            ...
            MediaSource.prototype.endOfStream = function() {
                push(rec.mediaSource, { t: t(), kind: 'endOfStream' });
                return origEndOfStream.apply(this, arguments);
            };
        } catch (err) { push(rec.errors, { t: t(), msg: 'mediasource wrap failed: ' + err.message }); }
        // Ad overlay watcher — Twitch's CSAI ad UI puts up the "Ad 1 of 2" header. Watch for it.
        try {
            const isAdOverlayNode = (node) => {
                if (!node || node.nodeType !== 1) return false;
            ...
            const observer = new MutationObserver((muts) => {
                for (const m of muts) {
                    for (const n of m.addedNodes) {
                        if (isAdOverlayNode(n)) {
                            push(rec.overlays, { t: t(), kind: 'overlay-mount', text: (n.textContent || '').slice(0, 200) });
                            return;
                        }
                    }
                }
        ...
        } catch (err) { push(rec.errors, { t: t(), msg: 'overlay watcher failed: ' + err.message }); }
        // Video element periodic snapshot — useful to see whether the player froze
        setInterval(() => {
            try {
                const v = document.querySelector('video');
                if (!v) return;
                // Only record while ads appear to be playing (overlay seen recently) OR ~5% sample
                const recentOverlay = rec.overlays.length > 0 && (t() - rec.overlays[rec.overlays.length - 1].t) < 60000;
                if (!recentOverlay && Math.random() > 0.05) return;
                push(rec.video, {
                    t: t(),
                    ...
                    buffered: v.buffered.length > 0 ? +v.buffered.end(v.buffered.length - 1).toFixed(2) : 0,
                });
            } catch {}
        }, 1000);
        // Public API
        window.vaftRecorder = rec;
        window.vaftDump = function(opts) {
            opts = opts || {};
            const json = JSON.stringify(rec, null, 2);
            const summary = '[vaftDump] ' + (json.length / 1024).toFixed(1) + ' KB — logs:' + rec.logs.length + ' fetches:' + rec.fetches.length + ' gql:' + rec.gqlOps.length + ' gqlAdResp:' + rec.gqlAdResponses.length + ' xhrs:' + rec.xhrs.length + ' mse:' + rec.mediaSource.length + ' overlays:' + rec.overlays.length + ' video:' + rec.video.length + ' blocked:' + rec.blockedRequests.length + ' errors:' + rec.errors.length;
            console.log(summary);
            if (opts.print) console.log(json);
            if (opts.download) {
                try {
                    ...
                    a.download = 'vaft-dump-' + (rec.vodIdAtStart || 'page') + '-' + stamp + '.json';
                    document.body.appendChild(a);
                    a.click();
                    setTimeout(() => { try { URL.revokeObjectURL(a.href); a.remove(); } catch {} }, 1000);
                    console.log('[vaftDump] downloaded ' + a.download);
                } catch (err) {
                    console.log('[vaftDump] download failed: ' + err.message);
                }
                return rec;
            }
            // Default: copy to clipboard. Async Clipboard API requires a user gesture
            // for some browsers when running from a userscript context; fall back to
            // execCommand on failure, and finally to a manual selectable textarea.
            ...
            const finish = (ok, method, err) => {
                if (ok) {
                    console.log('[vaftDump] copied to clipboard via ' + method + ' (' + (json.length / 1024).toFixed(1) + ' KB) — paste it back to share');
                } else {
                    console.log('[vaftDump] clipboard copy failed' + (err ? ' (' + err + ')' : '') + ' — fallback: window.vaftDumpText holds the JSON, or call window.vaftDump({download:true}) to save as a file');
                    window.vaftDumpText = json;
                }
            };
            try {
                if (navigator.clipboard && navigator.clipboard.writeText) {
                    navigator.clipboard.writeText(json).then(
                        () => finish(true, 'navigator.clipboard'),
                        (err) => {
                            const ok = copyViaExecCommand();
                            finish(ok, 'execCommand', err && err.message);
                        }
                    );
                    return rec;
                }
        ...
        window.vaftReset = function() {
            ['logs', 'gqlOps', 'gqlAdResponses', 'fetches', 'xhrs', 'mediaSource', 'overlays', 'video', 'blockedRequests', 'errors'].forEach(k => { if (rec[k]) rec[k].length = 0; });
            console.log('[vaftRecorder] reset');
        };
        console.log('[AD DEBUG] Diagnostic recorder enabled — call window.vaftDump() to download a report, window.vaftReset() to clear buffers');
    })();
})();

// ============================================================================
// TEIL 2: VOD Yoink Working! — v1.2.2 — Kernlogik (Klasse Yoink, Hilfsfunktionen)
// Diese Deklarationen erzeugen keine DOM-Zugriffe und können daher sicher
// direkt bei document-start ausgeführt werden, genau wie im Originalskript.
// ============================================================================
'use strict';
...
// ----------------------------------------------------------------------------
// VOD Yoink Init: Im Original lief dieser Block ohne @run-at, also im
// Tampermonkey-Standard "document-end" (DOM bereits vorhanden). Da das
// kombinierte Skript wegen des Ad-Blockers @run-at document-start nutzen muss
// (document.head existiert dort ggf. noch nicht), wird der Yoink-Init-Block
// hier unverändert in einen DOMContentLoaded-Guard verpackt. Dadurch startet
// er zum gleichen Zeitpunkt wie ursprünglich, ohne dass am eigentlichen
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', __yoinkInit);
} else {
  __yoinkInit();
}
...
// ============================================================================
// ============================================================================
// TEIL 3: PlayVod — Worker-Patch + Restriction Remover
// Der PlayVod-Code wurde auf die vom Benutzer bereitgestellte Variante ersetzt.
// Über das Steuerzentrum separat aktivierbar/deaktivierbar.
// ============================================================================
try {
    if (window.__twitchToolsSettings && !window.__twitchToolsSettings.get().playVodEnabled) {
        console.log('[PlayVod] per Twitch-Tools-Einstellung deaktiviert — skip.');
    } else {
        (function () {
            'use strict';
            ...
            window.Worker = class Worker extends oldWorker {
                constructor(twitchBlobUrl) {
                    ...
                    const blobUrl = URL.createObjectURL(new Blob([`
                        importScripts(
                            'https://cdn.jsdelivr.net/gh/hoernzilla/SlipStream-UserScript@main/src/patch_amazonworker.js',
                        );
                        ${workerString}
                    `]));
                    super(blobUrl);
            ...
            function initPlayVodRestrictionRemover() {
                // The original PlayVod code expects document.body to exist.
                if (!document.body) {
                    document.addEventListener('DOMContentLoaded', initPlayVodRestrictionRemover, { once: true });
                    return;
                }
                new RestrictionRemover();
            }

            initPlayVodRestrictionRemover();
        })();
    }
} catch (e) {
    console.error('[PlayVod] Initialisierung fehlgeschlagen:', e);
}
